...
# SwissJS Master Pipeline (Internal, Authoritative)

This document defines the end-to-end CI/CD strategy for SwissJS across branches, artifacts, and documentation classes. It is the source of truth for how code and documentation move from feature branches to public distribution.
...
- Cleanly separate audiences and content:
  ...
  - Internal engineering docs (directives, codegen specs) — never public.
...
- Zero-leak guarantee: internal-only content never reaches release or public.
...
- `docs/`
  ...
  - `docs/internal/` — internal directives, codegen specs, ADRs, architecture (never public)
...
- `tools/`
  ...
  - `tools/redactor/` — sanitizes public docs, removes internal markers
- `ci/`
...
- Internal docs — `docs/internal/`, `docs/development/`
  - Never ship. Block on `staging`/`release`/`mirror`.
- Public docs — `docs/public/`
  - Ship. Composed of:
    - Content in `docs/public/`
    - Generated API in `docs/api/`
    - Optionally, redacted internal content via `tools/redactor/` using markers:
      - `<!-- @internal-start --> ... <!-- @internal-end -->`
      - Single-line markers: `<!-- @todo-internal -->`
...
- Allow: devnotes/internal (still OK here).
- Gates:
  ...
  - Leak-prevention (warn): flag if public docs reference `@internal` markers
- Workflows:
...
- Block: devnotes/internal (no longer allowed in artifacts).
- Gates:
  - Same as `develop`, plus strict No‑Internal‑Content gate:
    - Fail if any path from `docs/internal/`, `docs/devnotes/`, `docs/development/` appears in the release bundle or docs site
...
- Enforced excludes (as of now):
  - `.git/`, `.github/workflows/`, `.github/ISSUE_TEMPLATE/`, `tools/`, `scripts/`, `etc/`, `docs/internal/`, `docs/devnotes/`, `docs/development/`
...
| Stage   | Devnotes/Internal | Redaction | Public Docs Ship | Mirror |
| ------- | ----------------- | --------- | ---------------- | ------ |
...
- Path policies & deny-lists enforced in CI scripts
  - Fail `staging`/`release` if internal paths appear in artifacts or docs site
- Redactor (`tools/redactor/`)
  - Strips `@internal` blocks/markers; fails if any residue remains
- Protected branches
  - `develop`, `staging`, `release` protected; required checks: `ci`, `policy-checks`, `docs`
- PR templates & labels
  - Require `pnpm reset` before PR
  - Labels: `docs:internal` (disallowed on staging/release), `docs:public` (triggers docs build)
...
1. `policy-checks.yml` — strict no‑internal gate for `staging`/`release`.
...
5. Labels automation for `docs:internal`/`docs:public`.

This plan is authoritative. Deviation requires an ADR in `docs/internal/` and approval.
