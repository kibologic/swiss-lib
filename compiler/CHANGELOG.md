...
- ff8cbb0: Fix `component X {}` syntax never getting its `SwissComponent` import
  ...
  _earlier_ in the same pipeline, so `hasClassWrapper` is already true by the
