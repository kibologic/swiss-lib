...
-  dependency bumps are set to `patch` to keep the graph consistent.
- Base branch: `develop`.
...
- Milestones per release/sprint (`<!-- MOVED TO INTERNAL: Phase 6 -->`, `6.1.0 Fix Pack`, etc.)
...
- Purpose
  - The private `release` branch is mirrored to the public repo `main` via a clean export (no private CI or -only files).
...
- How it works
  ...
  - Clean export: copies the repo to a `mirror/` directory while excluding:
    - `.git/`, `.github/workflows/`, `.github/ISSUE_TEMPLATE/`, `tools/`, `scripts/`, `etc/`, `docs//`
