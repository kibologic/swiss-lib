...
This page defines what is considered the stable, supported public API versus /private code paths.

- Public APIs are exported via top-level barrels and documented in `docs/api/`.
-  APIs are subject to change without notice and should not be imported by consumers.
...
Avoid importing from nested paths, which are  and may change.
