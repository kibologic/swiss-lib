...
import {
  ...
  hydrateDOM as hydrateDOMImpl,
} from "./hydration.js";
import { renderComponent as renderComponentImpl } from "./component-rendering.js";
import {
  createDOMNode as createDOMNodeImpl,
  createTextNode,
  createElementNode,
} from "./dom-creation.js";
import {
  updateDOMNode as updateDOMNodeImpl,
  updateTextNode,
  updateElementNode,
  updateComponentNode,
} from "./dom-updates.js";
...
export function renderToDOM(vnode: VNode, container: HTMLElement) {
  ...
  if (
    ...
    typeof container.innerHTML === "undefined"
  ) {
    devTools.error("[Renderer] renderToDOM: container is not an HTMLElement", {
      container,
      vnodeType:
        typeof vnode === "object" && vnode !== null && "type" in vnode
          ? (vnode.type as any)?.name
  ...
  try {
    ...
    if (
      ...
      devTools.log(
        `[Renderer] renderToDOM: Updating existing DOM node (vnode.dom found)`,
      );
    ...
    if (container.firstChild) {
        ...
        if (isComponentVNode(vnode)) {
          ...
          if (instance) {
            ...
            if (container instanceof HTMLElement) {
              ...
              devTools.log(
                `[Renderer] renderToDOM: Stored instance in containerToInstance map after replacing static content for ${instance.constructor.name}`,
              );
            }
          }
        }
      ...
      if (isComponentVNode(vnode)) {
        ...
        if (!existingInstance && container instanceof HTMLElement) {
          ...
          if (
            containerInstance &&
            containerInstance.constructor === vnode.type
          ) {
            existingInstance = containerInstance;
            devTools.log(
              `[Renderer] renderToDOM: Found instance from containerToInstance map: ${existingInstance.constructor.name}`,
            );
          }
        }

        // PRIORITY 1: Try to find existing instance from the existing DOM node.
        // We intentionally do not do recursive DOM walks here; instance tracking must be registry-based.
        if (!existingInstance) {
          existingInstance = componentInstances.get(existingDom as HTMLElement);
          devTools.log(
            `[Renderer] renderToDOM: Looked up instance from componentInstances map (existingDom): ${existingInstance ? existingInstance.constructor.name : "not found"}`,
          );
        }
        ...
        if (!existingInstance && oldVNode) {
          ...
          if (instanceFromOldVNode) {
            ...
            devTools.log(
              `[Renderer] renderToDOM: Found instance from oldVNode.__componentInstance: ${instanceFromOldVNode.constructor.name}`,
            );
          }
          // Also check if oldVNode itself is a component VNode
            ...
            if (instanceFromComponentVNode) {
              ...
              devTools.log(
                `[Renderer] renderToDOM: Found instance from oldVNode (component VNode): ${instanceFromComponentVNode.constructor.name}`,
              );
            }
          }
        }

        // We do not attempt to recover instances by scanning DOM subtrees.
        // If instance tracking is lost, it's safer to let updateComponentNode attempt a conservative update.
        ...
        if (existingInstance && existingInstance.constructor === vnode.type) {
          ...
          devTools.log(
            `[Renderer] renderToDOM: Found existing instance of ${existingInstance.constructor.name}, updating in place`,
          );
          ...
          if (container instanceof HTMLElement) {
            ...
            devTools.log(
              `[Renderer] renderToDOM: Stored instance in containerToInstance map for ${existingInstance.constructor.name}`,
            );
          }
          ...
          if (
            ...
            if (instanceFromOld) {
              ...
              if (container instanceof HTMLElement) {
                containerToInstance.set(container, existingInstance);
                devTools.log(
                  `[Renderer] renderToDOM: Stored instance in containerToInstance map (fallback) for ${existingInstance.constructor.name}`,
                );
              }
              // Update in place
              updateDOMNodeBound(existingDom, vnode);
        ...
        if (
          ...
          devTools.log(
            "[Renderer] renderToDOM: Same component type (by VNode type), updating in place",
          );
          if (typeof vnode === "object" && vnode !== null) {
            (vnode as { dom?: Node }).dom = existingDom;
          }
        ...
        isElementVNode(oldVNode) &&
        oldVNode.type === vnode.type
      ) {
        // Same element type - update in place
        devTools.log(
          "[Renderer] renderToDOM: Same element type, updating in place",
        );
        if (typeof vnode === "object" && vnode !== null) {
          (vnode as { dom?: Node }).dom = existingDom;
        }
      ...
      devTools.log(
        "[Renderer] renderToDOM: Different component/element or no match, replacing",
      );
      if (container != null && typeof container.innerHTML !== "undefined")
        container.innerHTML = "";
      const domNode = createDOMNodeBound(vnode);
      ...
      if (isComponentVNode(vnode)) {
        ...
        if (instance) {
          ...
          if (container instanceof HTMLElement) {
            containerToInstance.set(container, instance);
            devTools.log(
              `[Renderer] renderToDOM: Stored instance in containerToInstance map during initial render for ${instance.constructor.name}`,
            );
          }
        }
      }
    ...
    try {
      ...
      if (container != null && typeof container.innerHTML !== "undefined")
        container.innerHTML = `<div style="padding: 1rem; color: red;">Application Error: ${escapeHtml(errorMessage)}</div>`;
    }
...
export function renderToString(vnode: VNode): string {
  ...
  if (isComponentVNode(vnode)) {
    // Basic SSR for components: render component output and stringify.
    // This is not hydration-aware yet, but prevents components from rendering as empty.
    try {
